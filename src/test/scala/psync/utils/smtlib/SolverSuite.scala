...
"""
    ...
    val vars: List[Variable] = List(
      Variable("A").setType(FSet(psync.verification.Utils.procType)),
      Variable("i").setType(psync.verification.Utils.procType),
      Variable("v").setType(Int),
      Variable("n").setType(Int)
    )
    val decls: List[(psync.formula.Symbol, List[psync.formula.Type])] = List(
      //(declare-fun cardProcessID (Set_ProcessID_) Int)
      (Cardinality, List(psync.verification.Utils.procType)),
      ...
      (In, List(psync.verification.Utils.procType))
    )
