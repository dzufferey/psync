...
object Matching {
   ...
   * @param alsoBoolean (optional) consider predicates as generated terms (default is false)
   * @return subterms of f that will generated new ground terms when freeVariables are instantiated
   */
  def termsGeneratedBy( freeVariables: Set[Variable], f: Formula, alsoBoolean: Boolean = false): Set[Formula] = {
    assert(f.tpe == Bool, "termsGeneratedBy needs to be called at the boolean level")
    assert(f.boundVariables.intersect(freeVariables).isEmpty, "termsGeneratedBy does not yet support shadowing")
    def ok(f: Formula) = {
      (alsoBoolean || f.tpe != Bool) &&
      f.freeVariables.exists( v => freeVariables(v) ) &&
      (f match { case Variable(_) => false
                 case Application(s, _) => !FormulaUtils.symbolExcludedFromGroundTerm(s)
                 case _ => true })
    }
    def collect(acc: Set[Formula], f: Formula): Set[Formula] = {
      if (alsoBoolean || f.tpe == Bool)
        f match {
  ...
  def findLocalSubterms(cc: CC, freeVariables: Set[Variable], formula: Formula): Set[Map[Variable,Formula]] = {
    ...
    val notGeneratingVariables = freeVariables -- generators.flatMap(_.freeVariables)
    //for the variables not generating any new terms
    val ngv = notGeneratingVariables.toIndexedSeq
    val values = ngv.map( v => cc.groundTerms.filter(_.tpe == v.tpe).toIndexedSeq)
    val prod = Misc.cartesianProductIterator(values).map( vs => ngv.zip(vs).toMap ).toSet
    val maps1 = if (prod.isEmpty) Set(Map[Variable,Formula]()) else prod
    //this is for the term generating variables
    val emp = Set(Map[Variable,Formula]())
    val maps2 = generators.foldLeft(emp)( (acc, g) => {
      mergeMaps(acc, find(cc, freeVariables.intersect(g.freeVariables), g))
    })
    //
    mergeMaps(maps1, maps2)
  }
