...
class Group(val self: Short, val replicas: Array[Replica]) {
  ...
  def idToInet(processId: Short): InetSocketAddress = {
    replicas(processId).getNetAddress
  }

  def inetToId(address: InetSocketAddress): Short = get(address).id
...
object Group {
  ...
  def renameReplica(lst: List[Replica]): (List[Replica], Map[Short, Short]) = {
    val sorted = lst.sortWith( (a, b) => a.id < b.id ).zipWithIndex
    val idMap = sorted.foldLeft(Map.empty[Short,Short])( (acc, p) => acc + (p._1.id -> p._2.toShort))
    val renamed = sorted.map{ case (r, id) =>
        if (r.id != id.toShort) Replica(idMap(r.id), r.address, r.port) else r }
    (renamed, idMap)
  }

  def apply(self: Short, lst: List[Replica]): Group = {
...
class Directory(private var g: Group) {
  ...
  def idToInet(processId: Short) = g.idToInet(processId)
